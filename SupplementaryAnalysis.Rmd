---
title: "SupplementaryAnalysis"
output: html_document
date: "2024-08-28"
---
Code for supplementary analyses
...
scrna <- readRDS("/Users/gagled01/morganLab/Waldenstroms/singlecell/objects/SeuratAnalysis_ReclusteredObject.rds")
```
# Supplementary Figure 1 - Seurat object info
```{r}
# Supp figure 1A
...
```
# Supplementary Figure 2 - MBC surface markers 
```{r}
...
```
# Supplementary Figure 3 - patient recluster UMAPs
# load up ArchR object for
```{r}
library(ArchR)
library(Seurat)
library(ggplot2)
...
```
# Actually plotting Supp Fig 3 - TRUST4 clonality reclusters
```{r}
...
dev.off()
```
# Supp 3B - Reclustered patients by celltype
```{r}
...
```
# Supp figure 7 shows flow cytometry results and was therefore not analyzed here
...
```
# Supp figure 12 - Gene expression heatmap from bulk RNA-seq data. See proper script for that
# Supplementary Figure 13 - Tumor mutational burden figure. Not included here
# Suppementary Figure 14 - Cell cycle genes
```{r}
...
```
# Supplementary Figure 15 - NFkB signaling in B-cell differentiation
```{r}
...
```
# Supplementary Figure 16 - BCR signaling genes
```{r}
...
```
# Supplementary Figure 17 - inflammation/interferon/jak/stat signaling
```{r}
...
```
# Supplementary Figure 18 - PI3K/AKT/MTOR signaling
```{r}
...
```
# Supplementary Figure 19 - UPR and estrogen
```{r}
